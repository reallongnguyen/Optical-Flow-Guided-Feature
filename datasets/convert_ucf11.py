...
# The URL where the UCF11 data can be downloaded.
# _DATA_URL = ''
...
def _get_filenames_and_classes(dataset_dir):
    ...
    train_filesnames = dataset_utils.read_split_file(ucf_root, 'trainlist.txt')
    test_filesnames = dataset_utils.read_split_file(ucf_root, 'testlist.txt')

    return train_filesnames, test_filesnames, sorted(class_names)
...
def _get_dataset_filename(dataset_dir, split_name, shard_id):
    ...
    return os.path.join(dataset_dir, 'UCF11-tfrecord', output_filename)
...
def run(dataset_dir):
    ...
    if not tf.gfile.Exists(os.path.join(dataset_dir, 'UCF11-tfrecord')):
        tf.gfile.MakeDirs(os.path.join(dataset_dir, 'UCF11-tfrecord'))
    ...
    train_filenames, test_filenames, class_names = _get_filenames_and_classes(dataset_dir)
    ...
    num_train = int(math.ceil(len(train_filenames) * _RATIO_TRAIN))

    training_filenames = train_filenames[:num_train]
    if num_train < len(train_filenames):
        validation_filenames = train_filenames[num_train:]

    log_name = os.path.join(dataset_dir, 'UCF11-tfrecord', 'log_ucf11.txt')
    ...
    dataset_utils.write_label_file(labels_to_class_names, os.path.join(dataset_dir, 'UCF11-tfrecord'))

    # _clean_up_temporary_files(dataset_dir)
    print('\nFinished converting the UCF11 dataset!')
...
def test(dataset_dir):
    ...
    num_train = math.ceil(len(train_filenames) * _RATIO_TRAIN)
