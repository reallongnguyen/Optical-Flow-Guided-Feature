...
tf.app.flags.DEFINE_string(
    'train_dir', 'results/ucf11-off/train',
...
tf.app.flags.DEFINE_boolean(
    'use_off_sub_loss', True,
    'Su dung loss tier1 va tier2 o mang OFF.')
...
tf.app.flags.DEFINE_float(
    'weight_decay', 4e-4, 'The weight decay on the model weights.')

tf.app.flags.DEFINE_float(
    'resnet_weight_decay', 4e-3, 'The weight decay on the resnet model weights.')
...
tf.app.flags.DEFINE_float('learning_rate', 1e-6, 'Initial learning rate.')
...
tf.app.flags.DEFINE_string(
    'resnet_model_name', 'resnet_v2_26', 'The name of the architecture to train.')
...
tf.app.flags.DEFINE_string(
    'checkpoint_path', 'results/ucf11-rgb/train',
    'The path to a checkpoint from which to fine-tune.')

tf.app.flags.DEFINE_string(
    'checkpoint_exclude_scopes', 'OFF',
...
def main(_):
    ...
    batch_queue = slim.prefetch_queue.prefetch_queue(
        [inputs, labels], capacity=2 * deploy_config.num_clones)
    ...
    def clone_fn(batch_queue):
        ...
        if FLAGS.use_off_sub_loss:
            ...
            tf.losses.softmax_cross_entropy(
              logits=logits_tier1, onehot_labels=labels,
              label_smoothing=FLAGS.label_smoothing, weights=0.2, scope='off_tier1_loss')

            tf.losses.softmax_cross_entropy(
              logits=logits_tier2, onehot_labels=labels,
              label_smoothing=FLAGS.label_smoothing, weights=0.4, scope='off_tier2_loss')
